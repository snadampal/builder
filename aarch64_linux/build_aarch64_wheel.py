...
def build_ArmComputeLibrary(host: RemoteHost, git_clone_flags: str = "") -> None:
    ...
    host.run_cmd("cd ComputeLibrary && patch -p1 < $HOME/builder/mkldnn_fix/dynamic_quantization_acl.patch")
    host.run_cmd(f"cd $HOME/ComputeLibrary && scons Werror=1 -j8 {acl_build_flags}")
...
def start_build(host: RemoteHost, *,
    ...
    if host.using_docker():
        build_vars += " CMAKE_SHARED_LINKER_FLAGS=-Wl,-z,max-page-size=0x10000"
    if enable_mkldnn:
        host.run_cmd("cd $HOME && git clone https://github.com/snadampal/builder.git")
        host.run_cmd("cd $HOME/builder && git checkout aarch64_dyn_quantization")
        build_ArmComputeLibrary(host, git_clone_flags)
        print("build pytorch with mkldnn+acl backend")
        build_vars += " USE_MKLDNN=ON USE_MKLDNN_ACL=ON"
        host.run_cmd("cd $HOME/pytorch && patch -p1 < $HOME/builder/mkldnn_fix/dynamic_quantization_pytorch.patch")
        host.run_cmd("cd $HOME/pytorch/third_party/ideep && patch -p1 < $HOME/builder/mkldnn_fix/dynamic_quantization_ideep.patch")
        host.run_cmd("cd $HOME/pytorch/third_party/ideep/mkl-dnn && patch -p1 < $HOME/builder/mkldnn_fix/dynamic_quantization_onednn.patch")

        host.run_cmd("cd $HOME && rm -rf builder && git clone https://github.com/pytorch/builder.git")
